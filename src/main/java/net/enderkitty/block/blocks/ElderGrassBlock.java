...
public class ElderGrassBlock extends SnowyBlock implements Fertilizable {
    public ElderGrassBlock(Settings settings) {
        super(settings);
    }

    @Override public boolean isFertilizable(WorldView world, BlockPos pos, BlockState state) { return world.getBlockState(pos.up()).isAir(); }
    ...
    private static boolean canSurvive(BlockState state, WorldView world, BlockPos pos) {
        BlockPos blockPos = pos.up(); BlockState blockState = world.getBlockState(blockPos);
    ...
    private static boolean canSpread(BlockState state, WorldView world, BlockPos pos) { BlockPos blockPos = pos.up();
        return ElderGrassBlock.canSurvive(state, world, pos) && !world.getFluidState(blockPos).isIn(FluidTags.WATER); }

    @Override
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        if (!ElderGrassBlock.canSurvive(state, world, pos)) {
            world.setBlockState(pos, ArcaneDimsBlocks.ELDER_DIRT.getDefaultState());
            return;
        }
        if (world.getLightLevel(pos.up()) >= 9) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                if (!world.getBlockState(blockPos).isOf(ArcaneDimsBlocks.ELDER_DIRT) || !ElderGrassBlock.canSpread(blockState, world, blockPos)) continue;
